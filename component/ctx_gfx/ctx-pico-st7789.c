...
#endif
#if 0 // waveshare 240x240px joystick
  #define PIN_DIN 11
  #define PIN_CLK 10
  #define PIN_CS 9
  #define PIN_DC 8
...
// static allocation for scratch computations, number of
// pixels computed at once
#if PICO_RP2350
// we've got enough memory to favor the lack of inter-chunk
// tearing for complex frames over small memory use
...
{
  uint8_t caset_cmd[] = {5, 0, 0x2a, 0x00, 0x00, 0x00, 0xf0, 0};
  uint8_t raset_cmd[] = {5, 0, 0x2b, 0x00, 0x00, 0x01, 0x40, 0};
                           ...
                           int pin_reset, int pin_backlight, float clk_div)
{
  ...
  Ctx *ctx = ctx_new_cb (fb_width, fb_height, &config);
...
#if 0 // only applicable to the 240x240 buttons + joystick board
