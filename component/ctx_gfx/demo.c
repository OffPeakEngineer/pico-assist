...
typedef void (*SceneFun)(Ctx *ctx, const char *title, int frame_no,
                         float time_delta);
...
static void scene_logo(Ctx *ctx, const char *title, int frame_no,
                       float time_delta) {
  static int dir = 1;
  ...
  if (dir > 0) {
    dim += 10;
    if (dim > ctx_width(ctx) * 0.5)
      dir *= -1;
  } else {
    dim -= 10;
    if (dim < ctx_width(ctx) * 0.1)
      dir *= -1;
  }

  ctx_logo(ctx, ctx_width(ctx) / 2, ctx_height(ctx) * 0.4,
           ctx_height(ctx) * 0.6 +
               ctx_height(ctx) * 0.3 * sinf(frame_no / 10.0f));
}

static void scene_clock(Ctx *ctx, const char *title, int frame_no,
                        float time_delta) {
  ...
  if (radius > ctx_height(ctx) / 2)
    radius = ctx_height(ctx) / 2;
...
static void scene_curve_to(Ctx *ctx, const char *title, int frame_no,
                           float time_delta) {
  ctx_save(ctx);
  frame_no %= 400;
  clear(ctx);
  ...
  float x1 = 102.4 + ((frame_no % 60) - 52), y1 = 230.4, x2 = 153.6,
        y2 = 25.6 + (frame_no % 60), x3 = 230.4, y3 = 128.0;
  ...
  ctx_move_to(ctx, x2, y2);
  ctx_line_to(ctx, x3, y3);
  ctx_stroke(ctx);
  ctx_restore(ctx);
}
...
static void scene_drag(Ctx *ctx, const char *title, int frame_no,
                       float time_delta) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    case 2:
    case 3:
      ctx_rectangle(ctx, objects[i].x, objects[i].y, objects[i].width,
                    objects[i].height);
      break;
    case 4:
    ...
    case 5:
      ...
      ctx_line_to(ctx, objects[i].x + objects[i].width,
                  objects[i].y + objects[i].height);
      break;
    case 6:
    case 7:
      ctx_arc(ctx, objects[i].x + objects[i].width / 2,
              objects[i].y + objects[i].width / 2, objects[i].width / 2, 0.0,
              CTX_PI * 2.0, 0);
      break;
    }
    ctx_rgba(ctx, objects[i].red, objects[i].green, objects[i].blue,
             objects[i].alpha);
...
static void scene_spirals(Ctx *ctx, const char *title, int frame_no,
                          float time_delta) {
...
static void scene_spirals2(Ctx *ctx, const char *title, int frame_no,
                           float time_delta) {
  dot_count = frame_no + 16;
...
static void scene_spirals3(Ctx *ctx, const char *title, int frame_no,
                           float time_delta) {
  dot_count = frame_no + 16;
...
static void scene_circles(Ctx *ctx, const char *title, int frame_no,
                          float time_delta) {
  float width = ctx_width(ctx);
  float height = ctx_height(ctx);
  float alpha = 0.0;
  ...
  for (int i = 0; i < count; i++) {
    ctx_translate(ctx, sinf(scene_elapsed_time) * height * 0.4, 0);
    ctx_arc(ctx, width * 0.5, height * 0.5, height / 20 * i, 0.0, M_PI * 2, 0);
    ctx_translate(ctx, -2 * sinf(scene_elapsed_time) * height * 0.4, 0);
    ctx_arc(ctx, width * 0.5, height * 0.5, height / 20 * i, 0.0, M_PI * 2, 0);
    ctx_translate(ctx, sinf(scene_elapsed_time) * height * 0.4, 0);
  }
...
#if 1
char *slide_intro[] = {"UNUSEDI wanted good vectors in the terminal\n",
...
char *slide_intro1[] = {"native", "vectors", "are",      "rasterized", "as",
                        "late",   "as",      "possible", "ideally",    "on",
                        "the",    "GPU",     NULL};

char *slide_intro2[] = {"aUNUSED\n", NULL};
char *slide_intro3[] = {"bUNUSED\n", NULL};
char *slide_intro4[] = {"to cache or not to cache", NULL};

char *slide_protocol[] = {"protocol/API\n",
                          "  L 10 10\n",
                          "  lineTo 10 10\n",
...
char *slide_protocol1[] = {"protocol/API\n",
                           "  C 10 10 30 30 40 40\n",
...
char *slide_protocol2[] = {"  polylines, curves\n",
                           "  fill",
                           "and",
                           "stroke\n",
                           "  transforms\n",
...
char *slide_backends[] = {"backends\n",
                          ...
                          "hashcache,",
                          "rasterizer\n",
                          "Use ctx_width(ctx) and ctx_height(ctx) and adapt "
                          "the ui size to the provided viewport size\n",
                          NULL};
...
char *slide_backend_internals[] = {
    ...
    "\n",
    "  /* the following are only for event handling backends, and can be "
    "NULL*/ \n",
    ...
    "  void  (*destroy)         (void *backend); /* the destroy pointers are "
    "abused as the differentiatior\n",
    "                                               between different "
    "backends,   */\n",
    "  CtxFlags                  flags;\n",
    "  CtxBackendType            type;\n",
...
char *slide_backend_formatter[] = {"backend:formatter\n",
                                   "the reverse of the parser\n",
                                   "maybe rename to serialize\n", NULL};

char *slide_backend_ctx[] = {"backend:ctx\n", "interactive backend\n",
                             "ctx in terminal\n", "ctxevents for terminal\n",
                             NULL};

char *slide_parser[] = {"ctx_parse(Ctx *ctx, const char *string);\n"
                        "\n",
                        "  parses",
                        "the",
                        "text",
...
char *slide_rasterizer[] = {"rasterizer\n",
                            "active",
                            "edge",
                            "table",
                            "rasterizer\n",
...
char *slide_encodings[] = {
    "compositing", "code",   "target",     "pixel",
    "encodings\n", "\n",     "grayscale:", "1bit/2bit/4bit/8bit/float\n",
    "RGB",         "8bit",   "sRGB,",      "float,",
    "RGBA,",       "BGR565", "RGB332\n",   "CMYK",
    "8bit, float", NULL};
...
char *slide_backend_tiled[] = {"backend:tiled\n",
                               "subdivides screen in 6x5 grid\n", "updates\n",
                               "\n", NULL};

char *slide_backend_headless[] = {"backend:headless\n",
...
char *slide_backend_cb[] = {"backend:cb\n",
...
char *slide_backend_fb[] = {"backend:kms", "backend:fb\n", "\n",        "even",
                            "pid=1",       "is",           "possible!", NULL};

char *slide_backend_sdl[] = {"backend:SDL2\n",
...
char *slide_backend_pdf[] = {"backend:pdf\n", NULL};
char *slide_backend_term[] = {"backend:term\n", NULL};

char *slide_fonts[] = {"ctx has three fonts backends\n",
                       "  stb_ttf\n",
                       "  harfbuzz\n",
                       "  ctx,",
                       "which",
...
char *slide_mcu[] = {"card10 160x80\n",
                     ...
                     "rp2040 various\n",
                     "  ARM Cortex-M0",
                     "133mhz (OC 270mhz) ~512kb RAM\n",
                     "RISC-V 240x240 - ESP32c3\n",
                     "  160mhz ~400kb RAM\n",
                     NULL};

char *slide_events2[] = {" // make path .. ctx_move_to,",
                         "_line_to,",
                         "_close_path ();\n",
                         "ctx_listen",
                         "(ctx,",
...
char *slide_events3[] = {"static void action_fullscreen (CtxEvent *event, void "
                         "*data1, void *data2)\n",
                         "{\n",
                         " Ctx *ctx = event->ctx;\n",
...
static void scene_text(Ctx *ctx, const char *title, int frame_no,
                       float time_delta, float font_scale) {
  char **words = slide_default;
  if (!strcmp(title, "rasterizer"))
    words = slide_rasterizer;
  else if (!strcmp(title, "parser"))
    words = slide_parser;
  else if (!strcmp(title, "encodings"))
    words = slide_encodings;
  else if (!strcmp(title, "simd"))
    words = slide_simd;

  else if (!strcmp(title, "events2"))
    words = slide_events2;
  else if (!strcmp(title, "events3"))
    words = slide_events3;
  else if (!strcmp(title, "protocol"))
    words = slide_protocol;
  else if (!strcmp(title, "backends"))
    words = slide_backends;
  else if (!strcmp(title, "backend-cb"))
    words = slide_backend_cb;
  else if (!strcmp(title, "backend-pdf"))
    words = slide_backend_pdf;
  else if (!strcmp(title, "backend-internals"))
    words = slide_backend_internals;
  else if (!strcmp(title, "backend-internals2"))
    words = slide_backend_internals2;
  else if (!strcmp(title, "backend-formatter"))
    words = slide_backend_formatter;
  else if (!strcmp(title, "backend-ctx"))
    words = slide_backend_ctx;
  else if (!strcmp(title, "backend-term"))
    words = slide_backend_term;
  else if (!strcmp(title, "backend-hashcache"))
    words = slide_backend_hashcache;
  else if (!strcmp(title, "backend-displaylist"))
    words = slide_backend_displaylist;
  else if (!strcmp(title, "backend-tiled"))
    words = slide_backend_tiled;
  else if (!strcmp(title, "backend-headless"))
    words = slide_backend_headless;
  else if (!strcmp(title, "backend-fb"))
    words = slide_backend_fb;
  else if (!strcmp(title, "backend-sdl"))
    words = slide_backend_sdl;
  else if (!strcmp(title, "terminal"))
    words = slide_terminal;
  else if (!strcmp(title, "mcu"))
    words = slide_mcu;
  else if (!strcmp(title, "fonts"))
    words = slide_fonts;
  else if (!strcmp(title, "intro"))
    words = slide_intro;
  else if (!strcmp(title, "intro0"))
    words = slide_intro0;
  // else if (!strcmp (title, "intro1"))    words = slide_intro1;
  else if (!strcmp(title, "intro2"))
    words = slide_intro2;
  else if (!strcmp(title, "intro3"))
    words = slide_intro3;
  else if (!strcmp(title, "intro4"))
    words = slide_intro4;
  else if (!strcmp(title, "license"))
    words = slide_license;
  else if (!strcmp(title, "protocol1"))
    words = slide_protocol1;
  else if (!strcmp(title, "protocol2"))
    words = slide_protocol2;
  else if (!strcmp(title, "pippin"))
    words = slide_pippin;
  ...
  // if (y > height- font_size) scroll = scroll * 0.95 + 0.05 * (y -height +
  // font_size)  ; else scroll = 0;
}
...
static void scene_text_20(Ctx *ctx, const char *title, int frame_no,
                          float time_delta) {
  scene_text(ctx, title, frame_no, time_delta, 0.05);
}

static void scene_text_15(Ctx *ctx, const char *title, int frame_no,
                          float time_delta) {
  scene_text(ctx, title, frame_no, time_delta, 1.0 / 15.0f);
}
static void scene_text_13(Ctx *ctx, const char *title, int frame_no,
                          float time_delta) {
  scene_text(ctx, title, frame_no, time_delta, 1.0 / 13.0f);
}
static void scene_text_12(Ctx *ctx, const char *title, int frame_no,
                          float time_delta) {
  scene_text(ctx, title, frame_no, time_delta, 1.0 / 12.0f);
}

static void scene_text_10(Ctx *ctx, const char *title, int frame_no,
                          float time_delta) {
  scene_text(ctx, title, frame_no, time_delta, 0.1);
}

static void scene_text_7(Ctx *ctx, const char *title, int frame_no,
                         float time_delta) {
...
Scene scenes[] = {
    {"logo", scene_logo, DEFAULT_FRAMES},
    //{"pippin", scene_text_12, DEFAULT_FRAMES},
    {"spiral dots", scene_spirals, DEFAULT_FRAMES},
    {"spiral boxes", scene_spirals2, DEFAULT_FRAMES},
    {"spiral boxes", scene_spirals3, DEFAULT_FRAMES},
    {"circles", scene_circles, DEFAULT_FRAMES},
    {"curve_to", scene_curve_to, DEFAULT_FRAMES},
    //{"intro0", scene_text_12, DEFAULT_FRAMES},
    //{"terminal", scene_text_13, DEFAULT_FRAMES},
    //{"intro1", scene_text_7, DEFAULT_FRAMES},
    //{"intro2",      scene_text_12, DEFAULT_FRAMES},
    //{"intro3",      scene_text_12, DEFAULT_FRAMES},
    //{"protocol", scene_text_12, DEFAULT_FRAMES},
    //{"protocol1", scene_text_12, DEFAULT_FRAMES},
    {"protocol2", scene_text_12, DEFAULT_FRAMES},
    {"events2", scene_text_15, DEFAULT_FRAMES},
    {"events", scene_drag, DEFAULT_FRAMES},
...
static int ui_scenes(Ctx *ctx, void *data) {
  ...
  render_time = (ticks - prev_ticks) / 1000.0f / 1000.0f;
  ...
  scene_frames++;
  {
    ...
    ctx_save(ctx);
    scenes[scene_no].fun(ctx, scenes[scene_no].title, scene_frames,
                         render_time);
    ctx_restore(ctx);
    ...
    if (bouncy) {
      ...
      bx += bvx;
      by += bvy;
      if (bx < 0 || bx > ctx_width(ctx))
        bvx *= -1;
      if (by < 0 || by > ctx_height(ctx))
        bvy *= -1;
    }
